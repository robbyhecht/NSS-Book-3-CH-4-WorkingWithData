...
const githubData = [
  ...
  // -- loop through outer array, in each object find the commits array and grab its length --

// prepare to do the math at the end, counter variable set to zero
let totalCommits = 0;
// // loop through each event
githubData.forEach ( (event) => {
// // establish that we are only looking at specific combination of events
  if (event.payload.hasOwnProperty("commits")) {
// // do the math to get a single number using the variable
...
// set an empty array to hold data
let allEvents = []
// // loop through each data event (each object in the universal array)
githubData.forEach ( (event) => {
// // place the name of each event type into the empty array
  allEvents.push(event.type)
})
console.log(allEvents)
// // set an empty object
let eventCount = {}
// // for each event in the array, loop through events to see whether one exists
allEvents.forEach( (event) => {
// if it's in there, list it +1, if not, just add 1. forEach loops actually add the key name to the object first.
...
// use dot notation to mine into the data for if statement:
// githubData.payload.pull_request.merged === true
...
for(let i = 0; i < githubData.length; i++) {
 if (githubData[i].type === "PullRequestEvent") {
   if (githubData[i].payload.pull_request.merged === true && githubData[i].payload.pull_request.user.login !== "stevebrownlee")
...
// add to array:
// githubData.payload.pull_request.user.login = ...

// let uniqueUsers = [];
// githubData.forEach((event) => {
//   if (event.payload.hasOwnProperty("distinct") && githubData.payload.distinct[true]) {
//     uniqueUsers.push()
//   }
// })
...
// var count = 0;
// githubData.forEach(commits => {
//   count++
// });
// console.log(count)


// hasOwnProperty method -- payloads.hasOwnProperty("commits")
...
//   console.log(githubData[key].payload.commits);
// }
...
// }
// = 59 commits -- why no .length available?
// why can i console.log in the function, but not set a var and print that?

// console.log(Object.(githubData))
